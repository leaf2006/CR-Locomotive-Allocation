...
async def _first_emu_normal_xiaguan_data_processing(client: httpx.AsyncClient, url: str, store_dict: list, emu_xiaguan_compare: list):
    """发起首次emu_normal_fetch_xiaguan的网络请求，并把结果引入数据处理部分"""
    response = await client.post(url)
    response.encoding = "gb2312"
    get_webfetch_data(response.text, "emu_normal_fetch_xiaguan", url, store_dict, emu_xiaguan_compare)

async def _emu_normal_xiaguan_data_processing(client: httpx.AsyncClient, url: str, raw_result: list):
    """发起后续emu_normal_fetch_xiaguan的网络请求，并把结果引入数据处理部分"""
    response = await client.post(url)
    response.encoding = "gb2312"
    format_data(response.text, "emu_normal_fetch_xiaguan", raw_result)
...
async def first_web_fetch(fetch_url) -> list[str]:
    ...
    emu_normal_fetch_xiaguan = list(fetch_url['emu_normal_fetch'].values())
    ...
    first_raw_result = {}
    emu_xiaguan_compare = {} # 用于和emu_normal_fetch的raw_result数据进行比对与merge
    ...
    print("[SUCCESS] emu_normal_fetch组首次请求已完成")
    print("[INFO] 等待冷却三秒，进行emu_normal_fetch_xiaguan组的首次请求...")

    # emu_normal_fetch_xiaguan
    async with httpx.AsyncClient(timeout=30.0) as client:
        batches = []
        utils.fetch_divide(emu_normal_fetch_xiaguan, batches)

        for batch in batches:
            tasks = []
            for xiaguan_url in batch:
                if xiaguan_url == "None":
                    continue

                coro = _first_emu_normal_xiaguan_data_processing(client, xiaguan_url, store_dict, emu_xiaguan_compare)
                tasks.append(coro)
            await asyncio.gather(*tasks)
            await asyncio.sleep(1)
        

    return first_raw_result, store_dict, emu_xiaguan_compare

async def web_fetch(url_dict: list, raw_result :list, emu_xiaguan_compare: list) -> list[str]: # url_dict对应上面的store_dict
    """后续常规网络请求"""
    ...
    emu_normal_url_xiaguan_dict = url_dict['emu_normal_fetch_xiaguan']
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for batch in batches:
            tasks = []
            for url in batch:
                coro = _normal_data_processing(client, url, raw_result= raw_result)
                tasks.append(coro)
            await asyncio.gather(*tasks)
            await asyncio.sleep(1)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for batch in batches:
            tasks = []
            for url in batch:
                coro = _special_data_processing(client, url, raw_result= raw_result)
                tasks.append(coro)
            await asyncio.gather(*tasks)
            await asyncio.sleep(1)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for batch in batches:
            tasks = []
            for url in batch:
                coro = _emu_normal_data_processing(client, url, raw_result= raw_result)
                tasks.append(coro)
            await asyncio.gather(*tasks)
            await asyncio.sleep(1)

    print("[SUCCESS] emu_normal_fetch组请求已完成")
    print("[INFO] 等待冷却三秒，进行emu_normal_fetch_xiaguan组的请求...")
    await asyncio.sleep(3)

    # emu_normal_fetch_xiaguan
    async with httpx.AsyncClient(timeout=30.0) as client:
        batches = []
        utils.fetch_divide(emu_normal_url_xiaguan_dict, batches)
        ...
        for batch in batches:
            ...
            for xiaguan_url in batch:
                ...
                coro = _emu_normal_xiaguan_data_processing(client, xiaguan_url, raw_result= emu_xiaguan_compare)
                tasks.append(coro)
            await asyncio.gather(*tasks)
            await asyncio.sleep(1)
    
    return raw_result, emu_xiaguan_compare
